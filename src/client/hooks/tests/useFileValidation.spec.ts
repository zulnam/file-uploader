...
describe('useFileValidation', () => {
    ...
    it('should return a validation result with an error message for file size', () => {
        ...
        const validationResult = validationMethod(file);
        expect(validationResult).toEqual({ isValid: false, errorMessage: 'File "test.txt" exceeds the maximum size of 10485760 bytes' });
    });

    it('should return a validation result with an error message for file name length', () => {
        const { result } = renderHook(() => useFileValidation());
        const validationMethod = result.current;
        const file = new File(['test'], 'aaaaaaaaaafRYVuOZbOnOmEQvLfKAeP0fh6ez7_sxBRB97f1KqB8o9wC0M4f0DRchgveI-Nq5N9feSB1R0gDzVYCHkvvjw.igNnyDOSdbX-JsPxaxXKjJu2SiENNPh87Ha4j-4dGS8gNdjJ0uVIg2SJCnxvig6UN7M._2GAx0F4elrujUgvga4ST4dBp0_KcQIuAxb_EmAEEtldEnzMVXUSnWGF6hfhvm1hzpsCL8Fe7DQZ.3NOBgpL-FyZ1MyYtaj.txt', { type: 'text/plain' });
        const validationResult = validationMethod(file);
        expect(validationResult).toEqual({ isValid: false, errorMessage: 'File "aaaaaaaaaafRYVuOZbOnOmEQvLfKAeP0fh6ez7_sxBRB97f1KqB8o9wC0M4f0DRchgveI-Nq5N9feSB1R0gDzVYCHkvvjw.igNnyDOSdbX-JsPxaxXKjJu2SiENNPh87Ha4j-4dGS8gNdjJ0uVIg2SJCnxvig6UN7M._2GAx0F4elrujUgvga4ST4dBp0_KcQIuAxb_EmAEEtldEnzMVXUSnWGF6hfhvm1hzpsCL8Fe7DQZ.3NOBgpL-FyZ1MyYtaj.txt" name is too long (max 255 characters)' });
    });
    ...
    it('should return a validation result with an error message for invalid characters', () => {
        ...
        const validationResult = validationMethod(file);
        expect(validationResult).toEqual({ isValid: false, errorMessage: 'File "te/st@#$%^&*().txt" contains invalid characters. Only alphanumeric, dots, underscores, and hyphens are allowed' });
    });
    ...
    it('should return a validation result with an error message for executable file extension', () => {
        ...
        const validationResult = validationMethod(file);
        expect(validationResult).toEqual({ isValid: false, errorMessage: 'File "test.exe" has an executable extension (.exe) which is not allowed' });
    });
});